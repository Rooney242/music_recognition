...
for set_name, feat_train, feat_test, set_train, set_test in train_sets:
    ...
    models[set_name]['transitions']['best_params'] = rf_optuna.best_params

    best = ensemble.RandomForestRegressor(
        random_state=random_state,
        #criterion=models[set_name]['transitions']['best_params']['criterion'],
        max_depth=models[set_name]['transitions']['best_params']['max_depth'],
        #max_features=models[set_name]['transitions']['best_params']['max_features'],
        #min_impurity_decrease=models[set_name]['transitions']['best_params']['min_impurity_decrease'],
        min_samples_leaf=models[set_name]['transitions']['best_params']['min_samples_leaf'],
        #min_samples_split=models[set_name]['transitions']['best_params']['min_samples_split'],
        #min_weight_fraction_leaf=models[set_name][['transitions']'best_params']['min_weight_fraction_leaf'],
        n_estimators=models[set_name]['transitions']['best_params']['n_estimators'],
        ccp_alpha=models[set_name]['transitions']['best_params']['ccp_alpha']
    )
    best = ensemble.RandomForestRegressor()
    best = best.fit(feat_train, set_train)
    set_pred = best.predict(feat_test)
    models[set_name]['transitions']['adjusted_error'] = metrics.r2_score(set_test, set_pred)
    pickle.dump(best, open(mod_path+'trans_model_'+set_name+'.pkl', 'wb'))
...
for set_name, set_train, set_test in train_sets:
    ...
    models[set_name]['transitions']['adjusted_error'] = math.sqrt(metrics.mean_squared_error(set_test, set_pred))/ran(set_test)'''
